...
wait_module_ready() {
  kubectl wait module pod-reloader --for=condition=IsReady
  kubectl -n d8-pod-reloader wait deployment pod-reloader --for=jsonpath='{.status.readyReplicas}'=1
}


# --- Start ---
reset_state
kubectl apply -f $tests_path/test-on-create/mc-true.yaml
kubectl delete -f $setup_path/default-cm.yaml
sleep 1
wait_module_ready

echo "--- reloadOnCreate=true"
kubectl -n "$ns" annotate deployment "$deployment" pod-reloader.deckhouse.io/auto="true"

echo "- Deployment MUST be rolled out with created configmap"
kubectl -n "$ns" apply -f $setup_path/default-cm.yaml
sleep 1
if $debug; then
  debug_info
else
...
echo "--- reloadOnCreate=false"
reset_state
kubectl apply -f $tests_path/test-on-create/mc-false.yaml
kubectl delete -f $setup_path/default-cm.yaml
sleep 1
wait_module_ready

echo "- Deployment MUST NOT be rolled out with created configmap"
kubectl -n "$ns" apply -f $setup_path/default-cm.yaml
sleep 1
if $debug; then
  debug_info
