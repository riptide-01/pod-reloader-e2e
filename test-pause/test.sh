...
debug_info() {
  ...
  timeout 10s kubectl -n "$ns" get pods -w &
...
# --- Start ---
reset_state
echo "--- pod-reloader.deckhouse.io/pause-period=5s"
kubectl -n "$ns" annotate deployment "$deployment" pod-reloader.deckhouse.io/auto="true"
kubectl -n "$ns" annotate deployment "$deployment" pod-reloader.deckhouse.io/pause-period="5s"
sleep 5

echo "- Deployment MUST be rolled out with paused"
