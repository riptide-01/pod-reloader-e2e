...
# --- Start ---
reset_state
echo "--- pod-reloader.deckhouse.io/auto=true"
kubectl -n "$ns" annotate deployment "$deployment" pod-reloader.deckhouse.io/auto="true"

echo "- Deployment MUST be rolled out with changed configmap"
...
echo "- Deployment MUST be rolled out with changed secret"
...
echo "--- pod-reloader.deckhouse.io/auto=false"
kubectl -n "$ns" annotate deployment "$deployment" pod-reloader.deckhouse.io/auto="false"

echo "- Deployment MUST NOT be rolled out with changed configmap or secret"
kubectl -n "$ns" apply -f $modified_path/default-cm.yaml
kubectl -n "$ns" apply -f $modified_path/default-secret.yaml
sleep 1
if $debug; then
...
echo "--- configmap.pod-reloader.deckhouse.io/auto=true"
kubectl -n "$ns" annotate deploy "$deployment" configmap.pod-reloader.deckhouse.io/auto="true"
...
echo "- Deployment MUST NOT be rolled out with changed secret"
kubectl -n "$ns" apply -f $modified_path/default-secret.yaml
sleep 1
if $debug; then
  debug_info
...
echo "--- secret.pod-reloader.deckhouse.io/auto=true"
kubectl -n "$ns" annotate deploy "$deployment" secret.pod-reloader.deckhouse.io/auto="true"
...
echo "- Deployment MUST NOT be rolled out with changed configmap"
kubectl -n "$ns" apply -f $modified_path/default-cm.yaml
sleep 1
if $debug; then
  debug_info
